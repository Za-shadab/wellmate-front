...
import CollectionTab from "../planner/components/CollectionTab";
import DiscoverTab from "../planner/components/DiscoverTab";
...
const AlternativesView = ({ alternativeRecipes, currentRecipe, mealType, openRecipeUrl, getNutrientInfo, getMealTypeColor }) => {
  ...
  console.log("raw data:", recipes);
  // Parse the params to get clientId
  const parsedData = React.useMemo(() => {
    try {
      // First, safely decode the URI component
      let decodedString = recipes;
      try {
        decodedString = decodeURIComponent(recipes);
      } catch (decodeError) {
        console.log('Initial decode failed, trying alternative decode');
        // Try alternative decoding if needed
        decodedString = recipes.replace(/\+/g, ' ');
      }

      console.log('Attempting to parse:', decodedString);
      
      // Parse the decoded string
      const parsed = JSON.parse(decodedString);
      
      // Validate the parsed data structure
      if (!parsed.clientId || !parsed.mealType) {
        console.warn('Missing required fields in parsed data:', parsed);
      }
      
      return {
        clientId: parsed.clientId || null,
        currentRecipe: parsed.currentRecipe || null,
        alternateRecipes: parsed.alternateRecipes?.recipes || [],
  ...
  const handleSwapMeal = async (newRecipe) => {
    try {
      ...
      // Log the swap attempt details
      console.log('Attempting meal swap:', {
      ...
      if (response.data.success) {
        ...
        Alert.alert(
          ...
          [{ 
            text: "OK", 
            onPress: () => {
              // Always use replace to avoid stack issues
              router.back();
            }
          }]
        );
